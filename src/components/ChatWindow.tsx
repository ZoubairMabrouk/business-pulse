...
const FASTAPI_BASE = "http://localhost:8000"; // ← Adjust to your FastAPI URL
const TOP_K = 15;
...
async function sendToFastApi(prompt: string): Promise<string> {
  ...
  // Adapt to your FastAPI response shape — adjust field name if needed
  return data.mdx ?? data.answer ?? data.response ?? data.result ?? "Pas de réponse...";
}
...
export const ChatWindow: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Call FastAPI with body: { prompt: "...", top_k: 15 }
